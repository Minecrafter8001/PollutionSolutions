...
for _, tree in pairs(data.raw["tree"]) do
    ...
    tree.emissions_per_second = { pollution = -settings.startup["zpollution-tree-absorption"].value }
	end
end
...
data:extend(
{
    ...
    icon = "__PollutionSolutions2__/graphics/icons/pollution-collection.png",
		icon_size = 128,
...
{
	{
		type = "technology",
		name = "inceneration",
    icon = "__PollutionSolutions2__/graphics/icons/inceneration.png",
		icon_size = 128,
...
local function generate_barrel_item_icons(name, fluid, empty_barrel_item)
  ...
  {
    {
      icon = empty_barrel_item.icon or empty_barrel_item.icons[1].icon
    },
    {
...
local function create_barrel_item(name, fluid, empty_barrel_item)
  local result =
  {
    ...
    order = "b[" .. name .. "]",
    stack_size = (empty_barrel_item and empty_barrel_item.stack_size) or 10,
	fuel_category = "waste",
	burnt_result = empty_barrel_name,
...
addResistance(data.raw["curved-rail-a"],			POLLUTION_DAMAGE_TYPE, 100)
addResistance(data.raw["curved-rail-b"],			POLLUTION_DAMAGE_TYPE, 100)
addResistance(data.raw["half-diagonal-rail"],		POLLUTION_DAMAGE_TYPE, 100)
addResistance(data.raw["elevated-straight-rail"],			POLLUTION_DAMAGE_TYPE, 100)
addResistance(data.raw["elevated-curved-rail-a"],			POLLUTION_DAMAGE_TYPE, 100)
addResistance(data.raw["elevated-curved-rail-b"],			POLLUTION_DAMAGE_TYPE, 100)
addResistance(data.raw["elevated-half-diagonal-rail"],		POLLUTION_DAMAGE_TYPE, 100)
...
--data.raw["pipe"].collision_mask = {"item-layer", "object-layer", "water_tile"}
if settings.startup["zpollution-pipe-collision"].value == false then
  -- Factorio API expects a collision_mask with `layers` as a dictionary of booleans
  data.raw["pipe-to-ground"]["pipe-to-ground"].collision_mask = {
    layers = {
      ["item"] = true,
      ["object"] = true,
      ["water_tile"] = true,
...
if settings.startup["zpollution-pipe-vulnerable"].value == false then
  ...
  if not ptg.resistances then ptg.resistances = {} end
  for damageType, _ in pairs(data.raw["damage-type"]) do
    table.insert(ptg.resistances, {
      type = damageType,
      percent = 100
    })
  end
