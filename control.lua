...
function OnBuiltEntity(event)
	if IsToxicDump(event.entity) then
		AddToxicDump(event.entity)
	end
	if IsPollutionCollector(event.entity) then
		AddPollutionCollector(event.entity)
	end
end
...
function EntityDied(event)
	...
	if storage.nestsKilled == nil then storage.nestsKilled = 0 end
	if storage.spilledLoot == nil then storage.spilledLoot = {} end
	if storage.lootToCheck == nil then storage.lootToCheck = {} end
		...
		if safetyRadius == 0 or isArtillery then
			...
			for _,checkLoot in pairs(nearItems) do
				if checkLoot.stack.name == loot.name and checkLoot.to_be_deconstructed(event.force) == false then
					if storage.spilledLoot[checkLoot] == nil then
						checkLoot.to_be_looted = true
						storage.spilledLoot[checkLoot] = {force=event.force}
					end
				end
			end
		end
	...
	for checkLoot,info in pairs(storage.spilledLoot) do
		if not checkLoot.valid then
			--log("manually looted")
			storage.spilledLoot[checkLoot] = nil
		else
	...
	storage.lootToCheck = {}
	for checkLoot,_ in pairs(storage.spilledLoot) do
		storage.lootToCheck[checkLoot] = true
	end
	for checkLoot,_ in pairs(storage.lootToCheck) do
		if storage.lootToCheck[checkLoot] == true then
...
function AttemptMarkForPickup(thisLoot, safetyRadius)
	...
	-- remove myself and neighbors from storage.lootToCheck
	local halfRadius = safetyRadius / 2
	local stillToCheck = {}
	for checkLoot,v in pairs(storage.lootToCheck) do
		if storage.lootToCheck[checkLoot] == true then
			if Distance(thisLoot, checkLoot) < halfRadius then
				if not enemyNear then
					MarkForPickup(checkLoot)
				end
				storage.lootToCheck[checkLoot] = nil
			end
		end
	end
end
...
function MarkForPickup(thisLoot)
	if not thisLoot.valid then
		storage.lootToCheck[thisLoot] = nil
		storage.spilledLoot[thisLoot] = nil
		return
	end
	if storage.spilledLoot[thisLoot] == nil then
		--log("Loot at " .. math.floor(checkLoot.position.x) ..",".. math.floor(checkLoot.position.y) .. " is not in storage.spilledLoot")
		return
	end
	local force = storage.spilledLoot[thisLoot].force
...
function OnTick_ToxicDumps(event)
	if storage.toxicDumps == nil or not next(storage.toxicDumps) then return end
...
function OnTick_PollutionCollectors(event)
	if storage.collectors == nil or not next(storage.collectors) then return end
